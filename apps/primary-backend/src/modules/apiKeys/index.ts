...
router.put("/", authMiddleware, async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    return res.json(
      ApiKeyModel.updateApiKeyResponseSchema.parse({
        message: "Updated api key successfully",
      }),
    );
  } catch {
    return res.status(411).json(
      ApiKeyModel.disableApiKeyResponseFailedSchema.parse({
...
router.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      return res.json(
        ApiKeyModel.deleteApiKeyResponseSchema.parse({
          message: "Api key deleted successfully",
        }),
      );
    } catch {
      return res.status(411).json(
        ApiKeyModel.deleteApiKeyResponseFailedSchema.parse({
