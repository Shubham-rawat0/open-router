...
export  function ApiKeys() {
  ...
  const toggleMutation = useMutation({
    mutationFn: async ({ id, disabled }: { id: string; disabled: boolean }) => {
      const res = await api.put("/api", { id, disabled });
      return res.data;
    },
              ...
              <div className="flex items-start gap-2.5 text-sm text-emerald-400 bg-emerald-500/10 border border-emerald-500/20 rounded-lg px-3.5 py-3 mt-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-2">
                    <code className="text-xs bg-emerald-500/10 rounded px-2 py-1 font-mono truncate">
                      {newlyCreatedKey}
                    </code>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {apiKeys.map((key: any) => (
                    <tr
                      key={key.id}
                      className="border-b border-border/30 last:border-0 group text-muted-foreground"
                    >
